...
function deleteBlips()
	if FuelerBlips[1] ~= nil then
		for i=2, #FuelerBlips, 1 do
			RemoveBlip(FuelerBlips[i])
			FuelerBlips[i] = nil
		end
	end
end
...
AddEventHandler('phoenix_petroliere:hasExitedMarker', function(station, part, partNum)
	CurrentAction = nil
end)

AddEventHandler('phoenix_petroliere:hasEnteredMarker', function(station, part, partNum)
    ...
    while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) ) then
			if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
				TriggerEvent('phoenix_petroliere:hasExitedMarker', LastStation, LastPart, LastPartNum)
				hasExited = true
			end
			...
			TriggerEvent('phoenix_petroliere:hasEnteredMarker', currentStation, currentPart, currentPartNum)
		end
